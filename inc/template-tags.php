 ...
 * @package thim
 */
// ищи функцию  k_paging_nav в functions.php
...
if ( !function_exists( 'thim_entry_meta' ) ) :
	...
	function thim_entry_meta() {
        ...
        $theme_options_data['thim_show_author'] = 0; //todo: remove this line if show author
		?>
		<ul class="entry-meta">
			...
			if ( isset( $theme_options_data['thim_show_comment'] ) && $theme_options_data['thim_show_comment'] == 1 ) {
				...
				<?php if ( !post_password_required() && ( comments_open() || '0' != get_comments_number() ) ) :
					?>
					<!-- Начало вставленных блоков -->
					<!-- Проверка и вывод если поле есть -->
					<?php if (get_field('slojnost_acf')) {?>
					<li class="comment-total">
						<span>Сложность урока</span>
						<span class="value"><?php echo get_field('slojnost_acf') ?></span>
					</li>
					<?php }?>
					<!-- Проверка и вывод если поле есть -->
					<?php if (get_field('type_informacii_posta')) {?>
					<li class="comment-total">
						<span>Тип урока</span>
						<span class="value"><?php echo get_field('type_informacii_posta') ?></span>
					</li>
					<?php }?>
					<li class="comment-total">
						<span>Просмотров</span>
						<p><i class="fa fa-eye"></i>&#x2007;<span class="value"><?php   echo getPostViews(get_the_ID()); ?></span></p>
					</li>
					<li class="comment-total">
						<span>Оценка пользователями</span>
						<span><?php if(function_exists('the_ratings')) {the_ratings(); } ?></span>
					</li>
					<!-- Конец вставленных блоков -->
					<li class="comment-total">
